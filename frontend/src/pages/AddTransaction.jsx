    ...
    const fetchData = async () => {
      try {
        ...
        if (walletData.length > 0) {
          setWalletId(walletData[0]._id);
          if (walletData.length > 1) {
            setToWalletId(walletData[1]._id);
          } else {
            setToWalletId(walletData[0]._id);
          }
        }
      } catch (err) {
        console.error(err);
    ...
    if (type === 'transfer') {
      ...
      try {
        ...
        if (res.ok) {
          ...
          alert(errData?.error || 'Błąd przy wykonywaniu przelewu');
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-background pb-28">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 px-4 sm:px-6 pt-4 max-w-lg mx-auto w-full gap-5">
        ...
        {/* Tabs: WYDATKI / DOCHODY / PRZELEW (Sliding Pill) */}
        <div className="flex w-full bg-muted/60 p-1 rounded-xl relative select-none">
          <div 
            ...
            style={{
              ...
              transform: type === 'expense' ? 'translateX(0)' : type === 'income' ? 'translateX(100%)' : 'translateX(200%)'
            }}
          ></div>
          <button
            type="button"
          ...
          <button
            ...
            onClick={() => {
              setType('transfer');
              if (wallets.length > 1 && (!toWalletId || toWalletId === walletId)) {
                const other = wallets.find(w => w._id !== walletId);
                if (other) setToWalletId(other._id);
              }
            }}
            className={`relative z-10 flex-1 py-2.5 text-center text-xs sm:text-sm font-bold uppercase tracking-wider transition-colors ${type === 'transfer' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground/80'}`}
          >
            PRZELEW
          </button>
        </div>
        ...
        {/* Date & Wallet Selection Controls (or Transfer Form Controls) */}
        {type === 'transfer' ? (
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm flex flex-col justify-center">
                <span className="text-[11px] font-semibold uppercase text-muted-foreground tracking-wider mb-1">Z konta (ubędzie)</span>
                <Select value={walletId} onValueChange={(val) => {
                  setWalletId(val);
                  if (val === toWalletId && wallets.length > 1) {
                    const other = wallets.find(w => w._id !== val);
                    if (other) setToWalletId(other._id);
                  }
                }} required>
                        ...
                        if (selectedW) {
                          return (
                            <>
                              ...
                              <span className="truncate">{selectedW.name}</span>
                            </>
                          );
                        }
                        return <SelectValue placeholder="Wybierz portfel" />;
                      })()}
                    </div>
                  </SelectTrigger>
                  <SelectContent>
              ...
              {/* NA KONTO (Target) */}
              <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm flex flex-col justify-center">
                <span className="text-[11px] font-semibold uppercase text-muted-foreground tracking-wider mb-1">Na konto (przybędzie)</span>
                <Select value={toWalletId} onValueChange={setToWalletId} required>
                  <SelectTrigger className="border-none shadow-none p-0 h-auto bg-transparent focus:ring-0 text-foreground font-bold text-sm sm:text-base flex items-center justify-between">
                    <div className="flex items-center gap-2 overflow-hidden">
                      {(() => {
                        const selectedW = wallets.find(w => w._id === toWalletId);
                        if (selectedW) {
                          return (
                            <>
                              <div className="w-6 h-6 rounded-lg flex items-center justify-center text-white shrink-0 shadow-sm" style={{ backgroundColor: selectedW.color || '#3b82f6' }}>
                        ...
                        return <SelectValue placeholder="Wybierz portfel docelowy" />;
                      })()}
                    </div>
                  </SelectTrigger>
                  <SelectContent>
            ...
            {/* Date Picker for Transfer */}
            <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm flex flex-col justify-center">
              <span className="text-[11px] font-semibold uppercase text-muted-foreground mb-1 tracking-wider">Data przelewu</span>
              <Popover>
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm">
          ...
          <textarea 
            ...
            placeholder={type === 'transfer' ? "np. Przelew oszczędnościowy..." : "Dodaj opcjonalną notatkę do tej transakcji..."} 
            value={description}
        ...
        <button 
          type="submit" 
          disabled={!amount || !walletId || isSubmitting || (type === 'transfer' ? !toWalletId : !categoryId)}
          className={`w-full py-4 mt-2 font-bold rounded-2xl text-base shadow-xl flex items-center justify-center gap-2 transition-all ${
            !amount || !walletId || isSubmitting || (type === 'transfer' ? !toWalletId : !categoryId)
        ...
        >
          ...
          <span>{type === 'transfer' ? 'Wykonaj przelew' : 'Dodaj transakcję'}</span>
        </button>
