...
const PREDEFINED_COLORS = [
  '#3b82f6', '#0ea5e9', '#06b6d4', '#14b8a6', '#10b981', '#22c55e', '#84cc16', '#f59e0b', 
  '#f97316', '#ef4444', '#f43f5e', '#ec4899', '#d946ef', '#a855f7', '#8b5cf6', '#6366f1',
  '#64748b', '#78716c', '#000000'
];

const PREDEFINED_ICONS = [
  'CreditCard', 'Wallet', 'Landmark', 'Coins', 'Banknote', 'PiggyBank', 'DollarSign', 'Smartphone',
  'ShieldCheck', 'Briefcase', 'Building', 'Globe', 'Lock', 'TrendingUp', 'Award', 'Gift',
  'HeartPulse', 'Home'
];

const DynamicIcon = ({ name, ...props }) => {
  const IconComponent = LucideIcons[name] || LucideIcons['Wallet'];
  return <IconComponent {...props} />;
};

const AddWalletModal = ({ isOpen, onClose, onAdd, wallets = [] }) => {
  ...
  const customColors = Array.from(new Set([
    ...((wallets || []).map(w => w.color).filter(c => c && !PREDEFINED_COLORS.includes(c))),
      ...
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="sm:max-w-lg p-6 bg-background rounded-3xl border border-border/60 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
          <DialogHeader className="mb-2">
            <DialogTitle className="text-xl font-bold text-foreground">Nowy portfel / konto</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              Dodaj konto bankowe, gotówkę lub kartę z wybraną ikoną i kolorem.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm">
              <span className="text-[11px] font-semibold uppercase text-muted-foreground mb-1 block tracking-wider">Nazwa konta</span>
              <input 
                type="text" 
                placeholder="Np. Gotówka, mBank, PKO, Revolut..." 
                value={name} 
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-3 px-3.5 shadow-sm">
              <span className="text-[11px] font-semibold uppercase text-muted-foreground mb-1 block tracking-wider">Stan początkowy (zł)</span>
              <input 
                type="number" 
                step="0.01"
                placeholder="0.00" 
